...
<?php 
    if (isset($_POST['submit'])) {
        ...
        if(!in_array($file_type, ['jpg', 'jpeg', 'png', 'gif'])) {
            throw new CustomException("Invalid file type. Only JPG, JPEG, PNG, and GIF files are allowed.");
        }

            // Handle file upload
            if (move_uploaded_file($_FILES['img']['tmp_name'], $target)) {
                $w = "insert into login values(null,'$uname','$pas','user')";
                $t = insert($w);
                $w1 = "insert into users values(null,'$t','$fname','$lname','$plc','$phn','$email','$dob','$target')";
                $t1 = insert($w1);
                alert("Inserted successfully");
                return redirect("login.php");
            } else {
                throw new CustomException("Failed to upload the image.");
            }
...
<section class="registration-container">
    <section class="content-container">
        <center><h1>USER REGISTRATIONS</h1></center>
        <center class="bg">
            <form action="" method="post" class="form-control" enctype="multipart/form-data" id="registrationForm">
                <table align="center">
                    <tr>
                        <th>First name</th>
                    ...
                    <tr>
                        <th>Contact No</th>
                        <td><input type="number" class="form-control" max="9999999999" name="phn" placeholder="Phone No."  pattern="\d{10}" title="Please enter a valid 10-digit phone number" required>
                    </td>
                    </tr>
                    <tr>
                        <th>Email</th>
